...
- Scope: all bookings where the result is `>= 0`. Bookings with negative net cost (e.g. award stays where savings exceed $0 total cost) are clamped to $0 and placed in the `$0–50` bucket.

**Total/Night:**

- Value: `(Number(totalCost) * (Number(lockedExchangeRate) || 1)) / numNights`
- Scope: only bookings with `totalCost > 0`. Award stays ($0 total cost) are excluded — they carry no meaningful price signal.
- `lockedExchangeRate: null` (future non-USD bookings) falls back to `|| 1`, treating the foreign amount as USD. This is the same accepted compromise used throughout `page.tsx`.
