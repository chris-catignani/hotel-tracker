 ...
 *
 * Each test uses an isolated user so the dashboard only contains that user's
 * bookings, eliminating the race condition caused by the top-5 limit when
...
test.describe("Net Cost Consistency", () => {
  ...
  test("should show consistent Net/Night with loyalty earnings and elite bonus", async ({
    ...
    // The isolated user has no status, so we pass loyaltyPointsEarned explicitly.
    // Base rate is 5 pts/$. For $350 pretax: 350 * 5 = 1750 base pts.
    // 30% bonus: 1750 * 0.3 = 525 bonus pts. Total: 1750 + 525 = 2275 pts.
