...
async function main() {
  ...
  const pointTypeData = [
    ...
    },
    {
      ...
      programCurrency: "EUR",
      programCentsPerPoint: 0.02, // 2¢ EUR per point
    },
    {
  ...
  for (const pt of pointTypeData) {
    await prisma.pointType.upsert({
      ...
      update: {
        ...
        programCentsPerPoint: pt.programCentsPerPoint,
      },
      create: {
        id: pt.id,
        name: pt.name,
  ...
  await upsertSubBrands(HOTEL_ID.GHA_DISCOVERY, [
    ...
    { name: "Sunway" },
    { name: "TemptingPlaces" },
    { name: "The Bürgenstock Selection" },
  ...
  // Finalize checked-in bookings: lock exchange rates and loyalty points for past non-USD bookings
  await finalizeCheckedInBookings(ADMIN_USER_ID);
